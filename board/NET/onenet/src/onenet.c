...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Beep\":%d,", beep_info.Beep_Status);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Light\":%0.2f,", light_info.voltag);
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Xg\":%0.2f,", adxl362Info.x);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Yg\":%0.2f,", adxl362Info.y);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"Zg\":%0.2f", adxl362Info.z);
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	if(dataPtr != NULL)									//如果找到了
	{
		...
		else if(strstr((char *)req, "allled"))  //Added by YYB 2020-01-11 to control all LED simultaneously 
		{
			if(num == 1)
			{
				Led4_Set(LED_ON);
